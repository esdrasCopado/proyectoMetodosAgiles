...
const url_obtenerDetallesSorteoApartado = "/api/v1/numeroRifa/obtenerDetallesSorteosUsuario";
...
function generarSorteoPago(detalleSorteo) {
    ...
    const imagenUrl = `${hostUrl}/${detalleSorteo.ulrImagenSorteo.replace(/\\/g, "/")}`;

    const divCard = document.createElement("div");
    divCard.className = "cardSorteo";
    divCard.id =detalleSorteo.id; // ID único basado en el ID del sorteo
    divCard.innerHTML = `
            ...
            <div class="boton-pagar">
                <button class="btnPagar" onclick="pagarNumero(${detalleSorteo.id})">
                    <span>Pagar</span>
